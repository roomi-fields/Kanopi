...
- **REV-F01** `bloqué` [P1] — Stop/Pause inertes sur voix de code autonomes (.strudel/.hydra/.tidal/.p5) — playback.svelte.ts:97 ; regression __hush__ vers __stop_in_place__ non honoré (b61531e) ; le son continue sur Stop  _(bloqué: DEP: chantier Kronos-position, foyer KR-13 (voix de code Kronos, voie B = exposer un handle Kronos). Gele Romain.)_
- **REV-F02** `bloqué` [P1] — Eval voix-code = transport affiché STOPPED — real-core.ts:313 ; invariant 'eval qui sonne donc playing' non rétabli ; UI contredit l'audio (784fa44)  _(bloqué: DEP: idem F01, KR-13 (voie B). Le '-' BPM de starter-02 EST ce symptome. Gele Romain.)_
- **REV-F03** `bloqué` [P1] — STEP saute un temps sur deux — playback.svelte.ts:112 ; lit beatPosition brut (K+1) au lieu du compensé, moitié de la prod inaudible au pas-à-pas  _(bloqué: MODELE (Romain): Kronos POSSEDE machine d'etat + position (contrat). F03 = Kanopi appelle la COMMANDE step() (primitive KR-17 playWindow DEJA faite) + LIT l'etat, SUPPRIME l'arithmetique read+1 modulo. Chantier Kronos-position.)_
