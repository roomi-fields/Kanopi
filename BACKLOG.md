...
- **KAN-kro24** `fait` — Migration KRO-24 : retirer l appel HOTE a composeTreeModulations (composition CV) -> il MIGRE chez Kairos (qui detient l arbre, appelle la Couche-1 Kronos a l aplatissement). GO quand Kairos l a integre
...
- **KAN-24** `ouvert` [P2] — E4 (cross-repo, architecte) : le contrat hub kanopi-runtime-midi.md INTERFACE PINNÉE (2026-06-13) fige MidiSink, mais le dispatch VIVANT utilise MidiTransport (runtime-midi.d.ts:35 'canonical Kronos TransportLike for MIDI' ; bpx-adapter.ts:1747 new MidiTransport ; kronos-audio.ts:290). MidiSink déclaré mais NON utilisé → l'étalon fige la mauvaise surface. À réconcilier avec runtime-midi avant ratification du contrat MIDI
...
- **KAN-MUTE** `ouvert` — Bouton mute live du performeur : couche persistante d'etat utilisateur (semantique Mute de piste Ableton) survivant au stop->play, par-dessus l'armement compose, jamais effacee par reset()
...
- **STEP-A-SILENT** `ouvert` — BUG STEP-A : le bouton step avance la barre mais ne produit AUCUN son (emission d'evenements manquante au step ?)
- **STEP-B-DESYNC** `ouvert` — BUG STEP-B : play apres un step = pas de barre + le son saute plusieurs pas avant de reprendre (position/curseur desync post-step ; interaction replay() reset ?)
...
- **KAN-UX1** `ouvert` — UX Romain 2026-07-03 : bandeau droit — supprimer l'onglet Scenes (inutile) ; UN panneau Actors+Inspector en widgets empilés (pas des onglets), vu la densité d'infos
