...
- **KAN-75** `fait` [P1] — Exposer une scène indienne (raga/sargam) TESTABLE+audible dans la bibliothèque — fixtures BPscript/scenes/\*.bps jamais branchées  _(fait: Confirme par Kanopi sur sa propre mesure, par symbole (contre-mesure du 2026-08-14))_
- **KAN-76** `fait` [P2] — Hôte/transport : le driver doit CONTINUER à tiquer après pause() (sinon le gel différé CVA-1 ne se finalise pas) + retirer l'appel setStepGrain déprécié (kronos-audio.ts:548) _(fait: tick post-pause déjà correct (playback.svelte.ts:83-89, driver.stop seulement sur stop) ; setStepGrain retiré (kronos-audio.ts:548, commit local, ride prochain push))_
- **KAN-77** `fait` [P2] — Onglets du bas : ré-ordonner en 1=Structure (défaut sélectionné) / 2=Texte / 3=Console _(fait: Onglets bas = Structure·Texte·Console, défaut Structure (816df95 local). Bonus : id 'timeline'→'structure' canonisé (rétrocompat). svelte-check 0 err, persistance 2/2.)_
...
- **KAN-69** `fait` [P2] — Mon filtre d entrees de catalogue et le banc qui le garde tiennent sur la NATURE seule — un objet est une entree. Un PROTOTYPE DE TETE est un objet : il franchit les deux, et le banc reste VERT. Mesure du 2026-08-25 sur la forme predite par BPscript — 16 entrees sur son paquet actuel, 17 sur la forme predite, prototype compris, contre 16 annoncees. Le banc porte deja la phrase depuis le 2026-08-24 : « la nature seule ne suffit pas, elle est vraie par accident » — la frappe de BPscript retire l accident. Reparation : un critere de STRUCTURE, la trace de derivation publiee dans le paquet, dont le nom de champ n est pas encore connu. ⇒ ⛔ **ÉLARGI le 2026-08-30 par sa propre mesure** : la décision `object library` de Romain fait passer le filtre de UN objet non-entrée à **DEUX** — le prototype de famille ET l'entrée `library`. Éprouvé sur les deux formes, pas déduit : sur la forme d'aujourd'hui le filtre rend `[western, arabic]`, juste ; sur la forme visée il rend `[library, alphabet, western, arabic]` — **deux fantômes, sur 25 catalogues**. ⇒ ⛔ **Et ce n'est pas un rouge à la frappe : c'est un VERT sur des entrées fantômes**, la classe qui ne rougit jamais. ⚠️ Son propre commentaire le disait déjà — `resources.ts:92-94`, *« la nature seule est aujourd'hui vraie PAR ACCIDENT »* : **la décision met fin à l'accident**, en transformant chaque porteur de champ en objet. ⇒ Ce qui la ferme est un **discriminant PUBLIÉ dans le catalogue émis** — c'est l'objet de `decisions/2026-08-30-le-paquet-publie-le-type-de-chaque-entree.md`, dont le câblage est mesuré RESTANT dans la phase 3.  _(fait: Clos par l architecte le 2026-09-03 : l homonyme est tranche par toi — l item parlait de l entree 'library' DANS un catalogue, pas du mot du langage, et la decision section-name-version-type-sortent-library-tombe fait que cette entree n a jamais existe. Le fond est leve autrement : le filtre de NATURE n existe plus, resources.ts lit famille(...).entrees. Mesure : zero entree fantome sur tes neuf groupes, et ton banc de fiche retourne verifie la forme exacte du defaut d avant.)_
- **KAN-70** `ouvert` [P2] — **Aucune de mes 328 scènes ne nomme une voix du catalogue amont, donc le trajet complet n'est prouvé nulle part.** Mesuré par kanopi le 2026-08-30, en répondant au préavis de la réécriture des voix — pas en le cherchant. Sa complétion propose les quinze noms ; son corpus n'en exerce aucun. ⇒ bpscript prouve ses quinze corps chez lui, runtime-audio prouve les siens chez lui, et **personne ne mesure la chaîne catalogue → éditeur → dérivation → runtime → son**. ⇒ Le geste serait une scène de vitrine qui nomme une voix du catalogue, et un banc d'écran qui mesure son énergie. ⚠️ Non ouvert : kanopi ne l'ouvre pas d'elle-même, et l'architecte ne met aucun agent au travail. ⚠️ Et elle a retiré dans le même message une conséquence qu'elle avait annoncée à 13:08 — « ma complétion proposera quinze noms qui ne sonnent pas » — fausse dans les deux sens : la complétion propose des NOMS et non des corps, et les corps neufs sont réalisés. *La conséquence annoncée n'existait pas ; celle-ci est réelle et plus ancienne.*
- **KAN-71** `ouvert` — La liste des gelés d'une fenêtre inclut des voisins lus par un PAQUET ÉPINGLÉ, donc hors de leur arbre et immuables — rien de ce qu'ils font ne peut atteindre la mesure. Le discriminant existe déjà dans la librairie (la voie d'un chemin classe un chemin de dépôt de paquets), il n'est pas employé à la construction de la liste. La réparation exige un banc qui prouve les DEUX sens — un voisin épinglé sorti de la liste, un voisin vif gardé dedans — parce qu'un gel trop large coûte une discipline inutile tandis qu'un gel trop étroit fait porter un verdict sur deux états EN SILENCE.
