...
- **REV-F03** `bloqué` [P1] — STEP saute un temps sur deux — playback.svelte.ts:112 ; lit beatPosition brut (K+1) au lieu du compensé, moitié de la prod inaudible au pas-à-pas  _(bloqué: REOUVERT (Romain): marque fait a tort - NON CONFORME. L'hote calcule la position (read beatPosition +1 modulo n) au lieu de deleguer a kronos.step() (transport.ts:163 existe). A refaire via delegation Kronos = cross-repo (voie B). L'interim corrige le skip mais entérine le motif interdit.)_
...
- **REV-F13** `bloqué` [P2] — writeMmDirective ne réécrit que @mm, pas @tempo — mm-directive.ts:16 ; 100 pourcent hôte, changement BPM non réécrit sur scène v0.8  _(fait: b66f0c7 — MM_RE reconnaît @mm ET @tempo, mot-clé préservé, test vert)_  _(bloqué: REOUVERT (Romain): purpose OK (reecriture sur changement tempo UI, TransportCluster.svelte:84) MAIS garde @mm. @mm OBSOLETE doit DISPARAITRE. Refaire @tempo-SEUL dans le cluster elimination @mm (apres migration demos F17/F18).)_
