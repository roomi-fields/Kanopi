...
test('session 01 - strudel solo evaluates and produces audio', async ({ page }) => {
  ...
  // Inject via the dev-only `window.__kanopi` hatch (main.ts:25-35). The `.bps`
  // is self-contained: a single file, focused and evaluated in place.
  ...
  // A `.bps` is one runnable block (compiles to a BP3 grammar); Ctrl+Enter on
  // any line evaluates the whole session — the backtick voice is placed in time
  // and fires the Strudel adapter.
